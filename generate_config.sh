...
if test -f "Revolt.toml"; then
    if [[ $IS_OVERWRITING -eq 1 ]]; then
        if [ "$SECRETS_FOUND" -eq "0" ]; then
            ...
            echo "If you do not copy your existing secrets into secrets.env you WILL lose access to ALL of your files stored in your Veilgard instance."
            exit 1
        fi
        ...
        if [ "$SECRETS_FOUND" -eq "0" ]; then
            echo "Please configure secrets.env with your existing secrets to prevent losing access to your saved files in your Veilgard instance. You can see instructions on how to configure it by reading the file secrets.env.example. You can do this by running the command 'cat secrets.env.example'."
            echo "Overwriting your existing config will result in you losing access to all current files stored on your Veilgard instance unless you copy your old secrets into secrets.env."
        else
...
echo "Configuring Veilgard with hostname $DOMAIN"

VEILGARD_HOSTNAME="https://$DOMAIN"

read -rp "Would you like to place Veilgard behind another reverse proxy? [y/N]: "
if [ "$REPLY" = "y" ] || [ "$REPLY" = "Y" ]; then
    echo "Yes received. Configuring for reverse proxy."
    VEILGARD_HOSTNAME=':80'
...
# set hostname for Caddy and web variables
echo "HOSTNAME=$VEILGARD_HOSTNAME" > .env.web
