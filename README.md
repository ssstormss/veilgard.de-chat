...
Self-hosting Veilgard using Docker
</div>
<br/>

This repository contains configurations and instructions that can be used for deploying a full instance of Veilgard, including the back-end, web front-end, file server, metadata and image proxy, Veilgard branding overlay, and optional owner/admin moderation tools.
...
> [!NOTE]
> This Veilgard deployment is based on the AGPL-3.0 Stoat self-hosted stack. Keep `LICENSE`, `NOTICE.md`, and the `/source` route so your hosted instance stays legally clean.
...
- [Veilgard Owner and Admin Tools](#veilgard-owner-and-admin-tools)
- [Updating](#updating)
...
Now, pull in the Veilgard self-hosted configuration:

```bash
git clone <your-veilgard-repository-url> veilgard
cd veilgard
```
...
The generate_config.sh script will create the necessary secrets required to create a Veilgard instance, and the secrets will be inserted into a file named `secrets.env`. You should back up this file, as losing it may result in you losing access to all files on your Veilgard instance.
...
## Veilgard Owner and Admin Tools

This version includes optional moderation tools for the owner/admins:

- terminal commands for user, email, username, and IP bans
- optional Telegram bot commands
- optional localhost-only MongoDB UI
- a moderation gate in front of `/api` so banned IPs and login/register attempts receive a ban reason and duration

See [`docs/VEILGARD_ADMIN.md`](docs/VEILGARD_ADMIN.md) for setup and commands. Run this once after the database is online:

```bash
docker compose run --rm admin-tools init
```

Finally, start Veilgard. First, run it in the foreground with:
...
During configuration using `generate_config.sh` you will be asked if you'd like to place Veilgard behind another reverse proxy. Enter `y` to configure for reverse proxy. This will expose your caddy on port 8880, and you can reverse proxy to <http://localhost:8880>
...
Due to an incomplete implementation, Veilgard will kick any user that attempts to stream a video that is larger than the maximum size defined in `Revolt.toml`. If you would like to increase the maximum size allowed on your Veilgard instance, you can do so by modifying the `video_resolution` field under `[features.limits.new_user]` and `[features.limits.default]` to a larger value in your `Revolt.toml`. For example, 4k would be : `[3996, 2160]`.
...
> [!IMPORTANT]
> As of February 28, 2026, the configuration script will load secrets into `secrets.env`. You must copy your existing secrets into secrets.env to prevent `generate_config.sh` from overwriting your secrets. If your secrets are overwritten you will lose access to all files on your Veilgard instance.
>
